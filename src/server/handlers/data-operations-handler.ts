...
async function handleRead(filePath?: string): Promise<{ content: Array<{ type: string; text: string }>; isError?: boolean }> {
  try {
...
async function handleExport(format: string): Promise<{ content: Array<{ type: string; text: string }>; isError?: boolean }> {
  try {
